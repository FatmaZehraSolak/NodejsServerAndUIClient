...
const TBodyFunction = (TBodyArray, array) => {
  ...
  TBodyArray.forEach((genre, i) => {
    if (
      genre.capital.search(array) !== -1 ||
      genre.name.search(array) !== -1 ||
      genre.region.search(array) !== -1 ||
      genre.capital.toLowerCase().search(array) !== -1 ||
      genre.name.toLowerCase().search(array) !== -1 ||
      genre.region.toLowerCase().search(array) !== -1
    ) {
      $("#TBody").append(
        ' <tr> <th scope="row">' +
          i +
